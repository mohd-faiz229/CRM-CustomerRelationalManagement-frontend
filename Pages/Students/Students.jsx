  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black tracking-tight italic">Student Directory</h2>
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-[10px] font-black uppercase tracking-widest outline-none focus:border-blue-500 transition-all"
            value={searchTerm}
      ...
      <div className="border border-white/5 rounded-[1.5rem] shadow-2xl bg-[#0f172a]">
                ...
                <tr key={student._id} className="hover:bg-white/[0.01] transition-colors">
                  ...
                  <td className="p-5">
                    {editingId === student._id ? (
                      <div className="flex items-center gap-2 w-44 relative z-[100]">
                        ...
                        <button onClick={() => handleUpdate({ ...student, status: editedStatus })} className="p-2 text-emerald-500 bg-emerald-500/10 rounded-lg hover:bg-emerald-500/20 border border-emerald-500/20">
                          <FaCheck size={10} />
                        </button>
                        <button onClick={() => setEditingId(null)} className="p-2 text-rose-500 bg-rose-500/10 rounded-lg hover:bg-rose-500/20 border border-rose-500/20">
                          <FaTimes size={10} />
                        </button>
                      </div>
                    ) : (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all hover:brightness-125 ${getStatusStyle(student.status || "pending")}`}
                      >
                  ...
                  <td className="p-5 text-right">
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => setFullEditStudent(student)}
                        className="p-3 text-blue-400 hover:text-white bg-blue-400/5 hover:bg-blue-600 rounded-xl border border-blue-400/10 transition-all"
                        title="Edit Student"
                      >
                        <FaPen size={12} />
                      </button>
                      <button
                        ...
                        className="p-3 text-rose-400 hover:text-white bg-rose-400/5 hover:bg-rose-600 rounded-xl border border-rose-400/10 transition-all"
      ...
      {/* MODALS (Full Edit and Delete) remain as optimized previously */}
      <AnimatePresence>
        {fullEditStudent && (
          <div className="fixed inset-0 backdrop-blur-md bg-black/60 flex items-center justify-center z-[110] p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-lg bg-[#0f172a] border border-white/10 rounded-[2rem] p-8 shadow-2xl">
              <div className="mb-6">
                <h2 className="text-xl font-black text-white italic">Modify Record</h2>
      ...
      <AnimatePresence>
        {selectedStudent && (
          <div className="fixed inset-0 backdrop-blur-sm bg-black/70 flex items-center justify-center z-[130] p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0f172a] border border-white/10 rounded-[2.5rem] p-10 max-w-sm w-full text-center space-y-6 shadow-2xl">
              ...
              <div>
                <h2 className="font-black text-xl text-white italic">Permanently Remove?</h2>
                <p className="text-xs text-slate-500 mt-2 font-medium">Deleting <span className="text-white">{selectedStudent.name}</span>. This is irreversible.</p>
              </div>
